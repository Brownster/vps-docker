...
## MUST CHANGE THESE VARIABLES BEFORE BUILDING DOCKER FILE !! ##

#new linux user
ENV username changeme

# DYNDNS / noip host name that resolves into your vps ip address
ENV DYNDNS someplace.dydns-remote.com

# user name for accessing sickbeard, couchpotato ect
ENV WEBUSER webuser

# password for accessing all the web apps sickbeard, couchpotato ect
ENV WEBPASS webpass

# Username for Squid Proxy Server
ENV SQUIDUSER squid

#Please enter a password for Squid Proxy Server
ENV SQUIDPASS hideme

#  port for web squid proxy and expose it
ENV SQUIDPORT 7629
EXOPSE $SQUIDPORT
...
ENV FTPHOST somewhere.dyndns-remote.com
...
ENV FILMFTPDIR films

#TV ftp location
ENV TVFTPDIR tvseries

#Music ftp location
ENV MUSICFTPDIR music

#Books ftp location
ENV BOOKSFTPDIR ebooks

#Books ftp location
ENV GAMESFTPDIR games

#Books ftp location
ENV COMICSFTPDIR comics

#games mount location
ENV GAMESMNTDIR /home/media/games

#comics mount location
ENV COMICSMNTDIR /home/media/comics

#films mount location
ENV FILMMNTDIR /home/media/films

#tv series mount location
ENV TVMNTDIR /home/media/tv

#music mount location
ENV MUSICMNTDIR /home/media/music

#books mount location
ENV BOOKSMNTDIR=/home/media/books

#SABNZB port for web access
ENV SABPORT 7960
EXPOSE $SABPORT

#SICKBEARD port for web access
ENV SICKPORT 7961
EXPOSE $SICKPORT
...
#Headphones port for web access
ENV HEADPORT 7963
EXPOSE HEADPORT

#Lazy Librarian port for web access
ENV BOOKPORT 7964
EXPOSE $BOOKPORT

#Mylar port for web access
ENV MYLARPORT 7965
EXPOSE $MYLARPORT

#Gamez port for web access
ENV GAMESPORT 7966
EXPOSE $GAMESPORT
...
RUN	apt-get install -y git python-cheetah -y squid3 squid3-common fail2ban apache2-utils

# Modify fail2ban conf
RUN sed -i 's/enabled = false/enabled = true/' /etc/fail2ban/jail.conf \
...
# creating Diretcories
  && VOLUME /home/$username/.pid/
  && VOLUME /home/$username/temp
